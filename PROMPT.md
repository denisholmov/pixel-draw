...
Скопируй **весь файл** (Ctrl+A) и вставь в Agent-чат в пустой папке.

---

Создай проект hand-paint в текущей папке.

ПРАВИЛА (обязательно):
- Скопируй все файлы ниже ТОЧНО, символ в символ. Не переписывай, не рефактори.
- НЕ добавляй cv2.resize — только cap.set(1280, 720).
- Координаты: landmark.x * w, landmark.y * h где w,h = frame.shape[:2].
- paint_layer размером (720, 1280, 3). MediaPipe 0.10 tasks API, НЕ mp.solutions.hands.
- После создания файлов выполни: chmod +x setup.sh && ./setup.sh

Нужно пользователю: Python 3.10+, веб-камера, Linux/macOS, интернет на первый запуск.
...
☝️ рисует | ✊ стирает | 🖐 пауза | Q выход | C очистить
